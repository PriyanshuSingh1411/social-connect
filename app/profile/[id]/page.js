...
const [submittingComment, setSubmittingComment] = useState(false);
  ...
  const fetchUser = async () => {
    try {
      // This single response already includes both the profile and
      // that user's posts, so a separate fetchPosts() call against the
      // same endpoint was a fully redundant second request on every
      // profile load.
  ...
  const fetchFollowers = async () => {
    ...
    try {
      const res = await axios.get(`/api/users/${params.id}/followers`);
  ...
  const fetchFollowing = async () => {
    ...
    try {
      const res = await axios.get(`/api/users/${params.id}/following`);
