...
export default function ExplorePage() {
  ...
  const searchContent = async () => {
    try {
      ...
      // Search posts by hashtag. The real route is /api/hashtags/search
      // (this previously called the nonexistent /api/posts/hashtags,
      // which 404'd silently into an always-empty result).
      const hashtagRes = await axios.get(
        `/api/hashtags/search?q=${encodeURIComponent(searchQuery)}`,
      );

      setSearchResults({
        users: usersRes.data.users,
        posts: hashtagRes.data.posts || [],
      });
    } catch (error) {
            ...
            <div className={styles.section}>
              ...
              <div className={styles.postsGrid}>
                {searchResults.posts.map((post) => (
                  <Link
                    key={post._id}
                    href={`/profile/${post.userId?._id || post.userId}`}
